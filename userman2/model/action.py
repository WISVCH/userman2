...
class Action (LDAPConn):
    ...
    locked = property(_get_locked, _set_locked,
                      None, "Whether or not the action is locked")
...
def GetAllActions(filter_data=False, ld=None):
    ...
    if filter_data:
        ...
        filter_string += ")"
        res = ld.l.search_s(
            settings.LDAP_ACTIONDN, ldap.SCOPE_SUBTREE, filter_string)
    else:
...
def Add(actionName, host, affectedDN, description, parent=False):
    ...
    ld.addObject(
        dn, {
            'objectClass': 'chAction', 'cn': cn, 'actionName': actionName, 'host': host, 'actionLocked': 'TRUE',
