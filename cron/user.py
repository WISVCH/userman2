...
class User:
    ...
    def setHomeDirectory(self, host, homeDir):
        if host == 'ch.chnet':
            self.l.modify_s(
                self.dn, [(ldap.MOD_REPLACE, 'homeDirectoryCH', homeDir)])
            ...
            self.l.modify_s(
                self.dn, [(ldap.MOD_REPLACE, 'homeDirectory', homeDir)])
            res = self.l.search_s(self.dn, ldap.SCOPE_BASE)
            (_, attrs) = res[0]
            self.attrs = cidict(attrs)
            return self.attrs["homeDirectory"][0]
    ...
    def getPrimaryGroup(self):
        res = self.l.search_s(
            config.ldapGroupOU, ldap.SCOPE_SUBTREE, 'gidNumber=' + self.attrs["gidNumber"][0])
        if len(res) != 1:
            return "none"
        (_, attrs) = res[0]
        return attrs["cn"][0]
    ...
    def getSecondaryGroups(self):
        res = self.l.search_s(
            config.ldapGroupOU, ldap.SCOPE_SUBTREE, 'memberUid=' + self.attrs["uid"][0])
